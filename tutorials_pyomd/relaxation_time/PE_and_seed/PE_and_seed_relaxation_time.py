...
gro = mda.Universe('equi.tpr', 'equi.xtc')

#number of chains (polymers)
n_chains = 494
#number of seed chains
n_seeds = 70
...
for ts in mda.lib.log.ProgressBar(gro.trajectory, verbose=True,
    ...
    #skipping the seed chains; because python index starts at zero, the term
    #[n_seeds:] skips the chain 0 through 69
    chains = gro.atoms.fragments[n_seeds:]
...
popt, pcov = scipy.optimize.curve_fit(pyomd_math.expfunc, delta_time[:50], correlation_function[:50])
