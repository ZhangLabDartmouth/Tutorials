...
gro = mda.Universe('md_iPP18_400.tpr', 'md_iPP18_400.xtc')
...
monomer_C_length = 3  #check the select_atoms below before changing this parameter
######################## change these parameters ###############################
...
for ts in mda.lib.log.ProgressBar(gro.trajectory, verbose=True,
    ...
    backbones = [chain.select_atoms('name C*') for chain in chains]
    ########################change this parameters##############################
...
popt, pcov = scipy.optimize.curve_fit(pyomd_math.expfunc, delta_time[:30], correlation_function[:30])
...
ax.plot(delta_time,pyomd_math.expfunc(delta_time,popt),label=r'Curve Fit: $\tau=$'+'{:.1f}'.format(popt)+r'$\,\pm\,$'+'{:.2f}'.format(perr)+' ns')
